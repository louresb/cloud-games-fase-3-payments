...
Este documento descreve as configurações e detalhes da orquestração de containers utilizando Kubernetes para o serviço de `payments` da aplicação de microsserviços desenvolvida na Fase 2 do Tech Challenge da FIAP.
...
| template.spec.containers[0].envFrom | - `configMapRef.name: payments-config` and `secretRef.name: payments-secret` (carrega variáveis de ambiente do ConfigMap e do Secret) |
